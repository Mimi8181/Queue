...
void search(int data)
{
    if (isEmpty == 0)
    {
        for (int i = antrian.head; i <= antrian.tail; i++)
        {
            ...
            {
                cout >> "Data ada di index ke-" << i << endl;
            }
        }
    }
}

void edit(int indexDataUbah, int data)
{
    if (isEmpty == 0)
    {
        antrian.data[indexDataUbah] = data;
    }
}
...
int total(){
    int nilai = 0;
    for (int i = antrian.head; i <= antrian.tail; i++){
        nilai += antrian.data[i];
    }
    return nilai;
}

int terbesar(){
    int nilai = antrian.data[antrian.head];
    for (int i = antrian.head + 1; i <= antrian.tail; i++){
        if (antrian.data[i] >= nilai){
            nilai = antrian.data[i];
        }
    }
    return nilai;
